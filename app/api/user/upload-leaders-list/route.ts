    ...
    try {
        ...
        for (const user of values) {
            try {
               ...
               try {
                const {data} = await axios.post(`${baseUrl}/api/user/first-password`, {email: insertedUser.email});
               } catch (error) {
                console.log({errorr: "Error al enviar correo " + error});
               }
            } catch (error: any) {
        ...
        console.log("[SUPERVISORS_CREATE_MANY]", error);
