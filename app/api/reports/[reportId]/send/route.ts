...
import { Company, Driver, FatigueSleepReport, LogisticsCenter } from "@prisma/client"
            ...
            try {
                for (const admin of admins) {
                    await transporter.sendMail({
                        ...mailOptions,
                        to: admin.email,
                        ...generateEmailContent({ report: reportDB }),
...
const generateEmailContent = ({ report }: { report: FatigueSleepReport & { logisticsCenter: LogisticsCenter & { company: Company | null } | null; driver: { fullname: string | null } | null; city: { realName: string | null } | null } }) => {
    const baseUrl = process.env.NEXTAUTH_URL
    // const link = `${baseUrl}/${companyToken.token}`

    const driverName = report.driver?.fullname;
    const companyName = report.logisticsCenter?.company?.name;
